...
from model_lstm.AGCRNCell import AGCRNCell
import math
...
class AVWDCRNN(nn.Module):
    ...
    def forward(self, x, init_state, init_cell,node_embeddings):
        #shape of x: (B, T, N, D)
        ...
        for i in range(self.num_layers):
            ...
            for t in range(seq_length):
                state,cell,last_state = self.dcrnn_cells[i](current_inputs[:, t, :, :],current_last_inputs[:, t, :, :], state, cell,last_state,node_embeddings)
        ...
        for i in range(self.num_layers):
            ...
            inner_last_states = []
            for t in range(seq_length):
                state,cell,last_state = self.dcrnn_cells[i](current_inputs_short[:, t, :, :],current_last_inputs[:, t, :, :], state, cell,last_state,node_embeddings)
                inner_states.append(state)
                inner_last_states.append(last_state)
            output_hidden.append(state)
...
class AGCRN(nn.Module):
    ...
    def forward(self, source, targets, teacher_forcing_ratio=0.5):
        ...
        init_cell = self.encoder.init_hidden(source.shape[0])  #初始化层，（层数、批量、节点、维度）
        output, _,last_state,current_inputs_short= self.encoder(source, init_state, init_cell,self.node_embeddings)      #B, T, N, hidden
