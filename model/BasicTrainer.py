    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(data_loader):
                ...
                noise = (
                    (torch.rand_like(data) * 2 * args.policy_noise) - args.policy_noise
                )
                data = (data + noise).clamp(-args.max_data, args.max_data)
        ...
        np.save('./{}_{}_true.npy'.format(args.dataset,args.type), y_true.cpu().numpy())
        np.save('./{}_{}_pred.npy'.format(args.dataset,args.type), y_pred.cpu().numpy())
